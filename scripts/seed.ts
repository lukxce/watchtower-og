...
async function main() {
  ...
  for (const c of COMPETITORS) {
    await db.query(
      `INSERT INTO competitors (slug, name, domain, meta_page_id, youtube_handle, track_linkedin, queries, extra_tier1, extra_tier2)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
       ON CONFLICT (slug) DO UPDATE SET name=EXCLUDED.name, domain=EXCLUDED.domain, meta_page_id=EXCLUDED.meta_page_id,
      ...
      [c.slug, c.name, c.domain, c.meta_page_id, c.youtube_handle, c.track_linkedin, JSON.stringify(c.queries), JSON.stringify(c.extra_tier1), JSON.stringify(c.extra_tier2)],
    );
