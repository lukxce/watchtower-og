...
-- The brain's output: ONE whole-picture read per competitor — every signal,
-- move, buildout, and hiring pattern considered together — not per-signal
-- commentary (that approach was tried and rejected: it buried the feed in
-- text and reasoned about signals in isolation). Reads live on Battlecards
-- and Competitors; the Feed stays plain signals. Generated by Claude
-- in-session (scripts/reads.ts) or live via ANTHROPIC_API_KEY when set.
CREATE TABLE IF NOT EXISTS competitor_reasoning (
  competitor_id INT PRIMARY KEY REFERENCES competitors(id),
  hook TEXT NOT NULL,       -- one short line, used on dashboard cards
  narrative TEXT NOT NULL,  -- the full read, a few sentences max
  evidence JSONB,           -- [{label, text}] — the dated facts behind it
  generated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  generated_by TEXT NOT NULL DEFAULT 'claude-in-session'
);
`;
